...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::translate(model, glm::vec3(50, 50, 0.0));
		...
		//For each row
		for (int i = 0; i <= width/100; i++)//8
		{
			//For each column
			for (int j = 0; j <= height/100; j++)//6
			{
				model = glm::translate(model, glm::vec3(1, 0.0f, 0.0f));
				glUniformMatrix4fv(modelLoc, 1, FALSE, glm::value_ptr(model));
				glDrawArrays(GL_TRIANGLE_FAN, 0, 4);
			}
			model = glm::translate(model, glm::vec3(-7, 1, 0.0f));
			glUniformMatrix4fv(modelLoc, 1, FALSE, glm::value_ptr(model));
			glDrawArrays(GL_TRIANGLE_FAN, 0, 4);
		}
