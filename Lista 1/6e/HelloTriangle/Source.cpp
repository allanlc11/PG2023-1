...
//#endif
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Círculo", nullptr, nullptr);
...
int setupGeometry()
{
	...
	// Cada atributo do vértice (coordenada, cores, coordenadas de textura, normal, etc)
	// Pode ser arazenado em um VBO único ou em VBOs separados
	GLfloat vertices[NLADOS * 6];

	for (int i = 0; i < NLADOS; i++) {
		if (i % 2 == 0) {
			vertices[i * 6] = raio / 2 * cos(i * doisPi / NLADOS);
			vertices[i * 6 + 1] = raio / 2 * sin(i * doisPi / NLADOS);
			vertices[i * 6 + 2] = 0.0;

		}
		else {

			vertices[i * 6] = raio * cos(i * doisPi / NLADOS);
			vertices[i * 6 + 1] = raio * sin(i * doisPi / NLADOS);
			vertices[i * 6 + 2] = 0.0;
		}
		vertices[i * 6 + 3] = 1.0;
		vertices[i * 6 + 4] = 1.0;
